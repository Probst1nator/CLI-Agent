...
class UtilsManager:
    def __init__(self):
        import logging
        # Get the absolute path of the CLI agent directory
        # From: agent/utils_manager/utils_manager.py -> 
        self.cli_agent_root = Path(__file__).parent.parent.parent.absolute()
        self.utils_directory = "utils"
        self.utils_path = self.cli_agent_root / self.utils_directory
    ...
    def _load_utils(self) -> None:
        ...
        if not self.utils_path.exists():
            # The directory should exist, but if not, we can't proceed.
            print(colored(f"Utility directory not found at: {self.utils_path}", "red"))
            return

        # Scanning for utilities silently
        for file in self.utils_path.glob("*.py"):
            if file.name.startswith("_") or file.name == ".gitignore":
                continue

            try:
                # Convert file path to a Python module path 
                # e.g., /path/to/utils/updatefile.py -> tools.main_cli_agent.utils.updatefile
                module_name = f"tools.main_cli_agent.utils.{file.stem}"
...
## Usage: 
```python
from {util_cls.__module__} import {util_cls.__name__}
{run_docstring}
...
"""
    ...
    def get_relevant_tools_with_scores(self, query: str, top_k: int = 3) -> tuple[str, str]:
        """
        Generates a combined prompt with relevant tools and their similarity scores.
        This is used to augment the user's prompt and provide score visibility.

        Args:
            query (str): The user's request or query.
            top_k (int): The number of top relevant items to suggest.

        Returns:
            tuple[str, str]: (formatted prompt string, score info string) or empty strings if no relevant items found.
        """
        ...
        # Generate score info for display
        if relevant_tools:
            score_info = ", ".join([f"{r['name']}({r['score']:.3f})" for r in relevant_tools])

        prompt_result = self._build_tools_prompt(relevant_tools, relevant_guidance)
        return prompt_result, score_info
    ...
    def get_relevant_tools_prompt(self, query: str, top_k: int = 3) -> str:
        ...
        """
        prompt, _ = self.get_relevant_tools_with_scores(query, top_k)
        return prompt

    def _build_tools_prompt(self, relevant_tools: List[Dict[str, Any]], relevant_guidance: List[Dict[str, Any]]) -> str:
        """
        Build the tools prompt from relevant tools and guidance.
        """
        prompt: str = ""
                ...
                for tool in high_confidence_tools:
                    ...
                    try:
                        if hasattr(tool_class, '_run_logic'):
                            ...
                            tool_line = f"- **{tool_name}**: `from {module_name} import {tool_class.__name__}; {tool_class.__name__}.run{simplified_sig}` - {summary}"
                        else:
